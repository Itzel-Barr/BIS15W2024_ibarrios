...
Yes, these data are tidy. They are in long format.
...
#average_entero<- sydneybeaches_long%>%
  #separate(date, into=c("day", "month", "year"), sep = "/")%>%
  #group_by(site,year)%>%
  #summarize(meanenterococci=mean(enterococci_cfu_100ml, na.rm=T))
#average_entero
```

8. Make the output from question 7 easier to read by pivoting it to wide format.
```{r}
#average_entero_wide<- average%>%
...
```{r}
#select(average_entero_wide, "site", "2013")
```
