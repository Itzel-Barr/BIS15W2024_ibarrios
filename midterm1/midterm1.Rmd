...
```{r}
wolves%>%
  summarize(across(c(park, packcode), n_distinct))
```
...
```
# The druid pack had the highest pack size in 2001. In 2010, the pack size was zero which probably means the druid pack was loacted in another park. 
...
ynp%>%
  group_by(pack)%>%
  summarize(persisty1=1,
            total=n())
```
# The pack named Agate had the highest overall persistence.
# It is unique that this pack competes with the druid wolf pack for the same prey. The druid pack is from the previous question which had the largest pack size. 
...
```{r}
mollies_wolf <- wolves%>%
  select(park, pack, biolyr, packcode)%>%
  filter(pack=="mollies")%>%
  arrange(desc(biolyr))
mollies_wolf
```
