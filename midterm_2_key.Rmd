...
surgery %>% 
  ...
  labs(title="Participants by Race",
       x=NULL, # NULL means you do not want to title it 
       y="# participants")+
  theme_light()
```
...
```{r}
surgery %>% 
  filter(gender!="NA") %>% # "gender!" means it excludes the NA
  group_by(gender) %>% 
...
```{r}
surgery %>% #use a boxplot for range
  filter(gender!="NA") %>% 
  ggplot(aes(x=gender, y=age, fill=gender))+ # fill by what "x" is 
...
surgery %>% 
  ...
  facet_wrap(~asa_status)+ #When using "facet_wrap" include the "~"
...
surgery %>% 
  ...
  filter(ahrq_ccs %in% c("Colorectal resection", "Small bowel resection", "Gastrectomy; partial and total", "Endoscopy and endoscopic biopsy of the urinary tract", "Spinal fusion", "Nephrectomy; partial or complete")) %>% #All 6 rows instead of five since five overlap but one is an outlier so you add it 
  group_by(ahrq_ccs) %>% 
  summarise(ccsmort30rate=mean(ccsmort30rate),
...
```{r}
surgery %>% 
  ggplot(aes(x=ahrq_ccs, y=ccsmort30rate, group=ahrq_ccs))+ #Group by whatever "x" value is 
  geom_col(position="dodge", fill="steelblue", alpha=0.6)+ #geom_bar is usually only for counts and "x" values(working with one variable)
  coord_flip()+
