...
deserts%>%
  ...
  labs(title = "Number of Smaples for Species DM & DS",
       x="Year",
       y="n")
```
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "black", fill = "deeppink", bins=10)+
  labs(title = "Distribution of Body Mass")
```
...
homerange%>%
  ...
  geom_histogram(color="deeppink", fill="deeppink", alpla=0.3)+ 
...
homerange%>%
  ggplot(aes(x=log10.hra))+
  geom_histogram(aes(y=after_stat(density)), fill="deeppink", alpha=0.3, color="black")+
