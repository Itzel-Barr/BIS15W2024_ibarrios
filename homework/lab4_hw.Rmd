...
### The taxa represent the names of the variables in the data frame.
```{r}
...
```{r}
herbivores_homerange <- filter(homerange, trophic.guild=="herbivore")
herbivores_homerange
```
...
```{r}
mean(herbivores_homerange$mean.hra.m2, na.rm = T)
anyNA(herbivores_homerange)
```
...
```{r}
owls %>%
  select("family", "genus", "species", "mean.mass.g", "log10.mass", "common.name")
```
...
```
### The smallest owl is named Eurasian pygmy owl. They have prominent golden eyes and are mainly active during dusk or dawn. https://ebird.org/species/eupowl1
...
```
### The caracara is located near the Mexican border and Florida and is considered to be agressive as they may steal food from other birds. 
